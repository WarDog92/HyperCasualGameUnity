...
public class PlayerController_but : MonoBehaviour
{
    ...
    private void move()
    {
        if (_isGrounded && !lockMove)
        {
            ...
            moveVector = new Vector3(move_hor, 0, move_ver);
            if (!lockRotate)
            {
    ...
    void dash()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.R) && !lockDash)
            {
                lockDash = true;
                lockRotate = true;
    ...
    void dodge()
    {
        if (!(currentDashTime < maxDashTime))
        {
            if (Input.GetKeyDown(KeyCode.T) && !lockDodge)
            {
                lockDodge = true;
                lockRotate = true;
